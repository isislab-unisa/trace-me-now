...
class BleScanner {
	...
	constructor() {
		...
		console.log(`Room number... OK! trackingNode placed in room number ${roomNumber}`);
		console.log('Checking trackingNode ID... \n');

		trackingNodeId = process.env.UUID;

		if(trackingNodeId === undefined) {
			console.log('trackingNode ID not set up yet. Generating a new one... ');
		...
		console.log(`trackingNode ID... OK! trackingNode ID: ${trackingNodeId} \n`);
	...
	// Check if the received packet belongs to a device already seen or a new one
	...
	// Update the last seen time of an already existing device
	...
	#deleteDevices = function() {
		// Every minute scan every device seen so far and delete devices not seen for more then 5 minutes
		setInterval(() => {
			devices.forEach(device => {
	...
	// Write the change on an the JSON file
	#writeToFile = function() {
		let data = JSON.stringify(devices, null, 2);
		fs.writeFile('root/devices.json', data, (err) => {
			if (err) {
					throw err;
			}	
		});	
